...
(() => {
    ...
    Game_Actor.prototype.checkEquipRequiriments = function(actor, item) {
        ...
        let match;
        while ((item) && (match = reg.exec(item.note))) {
            switch (match[1]) {
                case "level":
                ...
                case "luck":
                    ...
                    break;
                case "armor":
                    if (result) result = actor.hasArmor($dataArmors[match[2]]);
                    break;
                case "actorId":
                    if (result) result = actor._actorId == match[2];
                    break;
                case "classId":
                    if (result) result = actor._classId == match[2];
                    break;
                case "dclassId":
                    if (result) result = (actor._classId == 2) || (actor._classId == 11);
                    break;
                case "switch":
